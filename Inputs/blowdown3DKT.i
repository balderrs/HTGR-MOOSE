...
[FVBCs]
  [x_pressure_walls]
    ...
    variable = rhou
    momentum_component = x
    boundary = 'RPVIW RPVOW PCU ContIW CavitySP RPVSP'
  []
  [y_pressure_walls]
    type = PCNSFVImplicitMomentumPressureBC
    variable = rhov
...
[Outputs]
  ...
  interval = 100  # only output every 100 timesteps
[]
[Postprocessors]
  [cfl_dt]
  ...
  [TC01]
    type = PointValue
    point = '-0.061912 0.898525 0.254'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC02]
    type = PointValue
    point = '-0.468315 0.898525 0.254'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC03]
    type = PointValue
    point = '-0.061912 0.492125 0.254'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC04]
    type = PointValue
    point = '-0.468315 0.492125 0.254'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC05]
    type = PointValue
    point = '-0.061912 0.098425 0.254'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC06]
    type = PointValue
    point = '-0.468315 0.098425 0.254'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC07]
    type = PointValue
    point = '-0.468315 1.177925 0.254'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC08]
    type = PointValue
    point = '-0.2651125 1.177925 0.254'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC09]
    type = PointValue
    point = '-0.061912 1.177925 0.254'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC10]
  ...
  [TC11]
    type = PointValue
    point = '-0.2651125 0.898525 0.254'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC12]
  ...
  [TC14]
    type = PointValue
    point = '-0.2651125 0.492125 0.254'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC15]
  ...
  [TC17]
    type = PointValue
    point = '-0.2651125 0.098425 0.254'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC18]
  ...
  [TC20]
    type = PointValue
    point = '-0.061912 1.267 0'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC21]
    type = PointValue
    point = '-0.2651125 1.267 0.254'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC22]
    type = PointValue
    point = '-0.468315 1.267 0'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC23]
    type = PointValue
    point = '-0.601662 0.425133 0'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC24]
    type = PointValue
    point = '-0.601662 0.247333 0'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC25]
    type = PointValue
    point = '-0.6524625 1.4317472 0'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC26]
    type = PointValue
    point = '-1.152525 1.4317472 0.254'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC27]
    type = PointValue
    point = '-1.152525 0.4572 0.254'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC28]
    type = PointValue
    point = '-1.152525 -0.3556 0.254'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [TC31]
    type = PointValue
    point = '-1.194329 1.2430985 0'
    variable = temperature
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [VT08]
    type = PointValue
    point = '-0.563562 0.425133 0'
    variable = U
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [VT10]
    type = PointValue
    point = '-0.563562 0.247333 0'
    variable = U
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [VT12]
    type = PointValue
    point = '-1.228196 1.2430985 0'
    variable = U
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [Pressure_RPV]
